...
## Anexo 2. Otras metodologías para selección de los DEGs:
...
```{r, warning=FALSE}
res.selectCox <- selectCox(Surv(t, status) ~ ENSG00000167749.11 + ENSG00000046774.10 +ENSG00000010282.15 +
                   ENSG00000267774.2+ENSG00000166105.16+ENSG00000215117.6+ENSG00000223350.2+
                   ENSG00000077009.13+ENSG00000105141.6+ENSG00000181903.5+ENSG00000256292.2+
                   ENSG00000145192.13+ENSG00000268621.6+ENSG00000260542.1+ENSG00000234844.1+
                   ENSG00000268879.1+ENSG00000174562.13+ENSG00000133020.4+ENSG00000043355.12+
                   ENSG00000223786.1+ENSG00000157005.4+ENSG00000227555.1+ENSG00000235207.1+
                   ENSG00000118271.12+ENSG00000249306.6+ENSG00000133742.14+ENSG00000136110.13+
                   ENSG00000269729.1+ENSG00000171431.4+ENSG00000214978.7+ENSG00000255951.1+
                   ENSG00000227744.4+ENSG00000236366.3+ENSG00000189001.11+ENSG00000248227.1+
                   ENSG00000287583.1+ENSG00000248719.2+ENSG00000238194.2+ENSG00000139800.8+
                   ENSG00000169876.14+ENSG00000158874.11+ENSG00000176826.15+ENSG00000164363.10+
                   ENSG00000286016.1+ENSG00000250149.2+ENSG00000119919.11+ENSG00000155269.12+
                   ENSG00000249407.1+ENSG00000170044.8+ENSG00000155269.12+ENSG00000249407.1+
                   ENSG00000170044.8+ENSG00000102854.16, data =  matriz_cox, rule = "aic" )
```

- Veamos qué genes ha incluído en el modelo con esta selección backward:
...
- Verifiquemos si nuetros 7 DEGs han sido seleccionados o no por este modelo:
...
# summary(km_fit)
 plot(km_fit, xlab="años", lty = c("solid", "dashed"), col = c("black", "grey"),main = "Curva KM conjunto TEST modelo EXPRESIÓN GENÉTICA")
 legend("bottomleft", c("Riesgo alto (n=26)", "Riesgo bajo (n=63)"), lty = c("solid", "dashed"), col = c("black", "grey"))
 
logRank <- survdiff(Surv(t/365, status) ~ risk, data = union_test_gen)
pval <- p.val <- 1 - pchisq(logRank$chisq, length(logRank$n) - 1) 
pval
```
Como podemos observar, ni las curvas KM ni el log-rank obtenidos son mejores que el modelo seleccionado sólo con 7 DEGs, por lo que seguimos manteniendo este como nuestro modelo preferido. 
Lo que sí hemos podido verificar es que esta forma de seleccionar las variables del modelo incluyen a los 7 DEGs del nuestro.
...
library(MASS)
```
- Creamos el modelo y aplicamos la función stepAIC con el parámetro direction=both (es una metodología combinada entre backward y fordward):
...
fitb$formula
```
Traspasamos los DEGs seleccionados en el modelo final a un vector:
...
## Anexo 3. Ubicación en github

Este documento queda alojado en: 
https://github.com/alejandrofp-hub/TFM-Material-complementario/tree/main
